...
export const getStaticProps = async ({ params }: { params: { id: string } }) => {
  try {
    const filter = 'flag,name,nativeName,topLevelDomain,population,currencies,region,subregion,languages,capital,borders';
    const id = encodeURIComponent(params.id);

    const res = await fetch(`https://restcountries.com/v2/name/${id}?fullText=true,${filter}`);
